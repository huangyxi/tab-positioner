...
test.describe('Popup Behavior', () => {
	test('should handle popup creation by moving it to normal window', async ({ context, extensionId, configureSettings, getTabs }) => {
		...
		const popupTab = tabs.find(t => t.url.includes('example.com'));
